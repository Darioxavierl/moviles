...
class Encoder:
    ...
    def _spread_spectrum_vectorized(self, 
                                    message_bipolar: np.ndarray, 
                                    code: np.ndarray) -> np.ndarray:
        """
        Versión vectorizada del esparcimiento espectral (más eficiente).
        
        Usa broadcasting de numpy para evitar loops explícitos.
        """
        # Reshape para broadcasting: (n_bits, 1) * (1, code_length)
        message_reshaped = message_bipolar.reshape(-1, 1)
        code_reshaped = code.reshape(1, -1)
        
        # Multiplicación con broadcasting
        spread_matrix = message_reshaped * code_reshaped
        
        # Flatten a vector 1D
        signal = spread_matrix.flatten()
        
        return signal
    
    def add_noise(self, signal: np.ndarray, snr_db: float) -> np.ndarray:
        """
        Agrega ruido AWGN (Additive White Gaussian Noise) a la señal.
        ...
        Example:
            >>> encoder = Encoder()
            >>> clean_signal = np.array([1, -1, 1, -1])
            >>> noisy_signal = encoder.add_noise(clean_signal, snr_db=10)
        """
        # Calcular potencia de la señal
        signal_power = np.mean(signal ** 2)
        
        # Calcular potencia del ruido desde SNR
...
if __name__ == "__main__":
    # Ejemplo de uso
    print("=== Codificador CDMA ===\n")
    
    # Crear codificador
    encoder = Encoder()
    
    # Ejemplo 1: Codificar un solo usuario
    print("Ejemplo 1: Usuario individual")
    print("-" * 50)
    message = np.array([1, 0, 1, 1])  # 4 bits
    code = np.array([1, -1, 1, -1])    # Código de longitud 4
    
    print(f"Mensaje original: {message}")
    print(f"Código de esparcimiento: {code}")
    
    signal = encoder.encode_single_user(message, code)
    print(f"Señal codificada (longitud {len(signal)}): {signal}")
    print(f"Factor de esparcimiento: {encoder.get_spreading_factor(len(code))}")
    print(f"Ganancia de procesamiento: {encoder.get_processing_gain_db(len(code)):.2f} dB\n")
    
    # Ejemplo 2: Codificar múltiples usuarios
    print("Ejemplo 2: Múltiples usuarios (sistema CDMA)")
    print("-" * 50)
    n_users = 3
    n_bits = 4
    
    # Mensajes de 3 usuarios
    messages = np.array([
        [1, 0, 1, 0],  # Usuario 1
        [0, 1, 1, 0],  # Usuario 2
        [1, 1, 0, 1]   # Usuario 3
    ...
    # Codificar todos los usuarios
    signals = encoder.encode_all_users(messages, codes)
    
    print(f"\nSeñales codificadas (forma: {signals.shape}):")
    for i, sig in enumerate(signals):
        print(f"  Usuario {i+1}: {sig}")
    
    # Combinar señales (canal CDMA)
    total_signal = encoder.combine_signals(signals)
    print(f"\nSeñal total en el canal:")
    print(f"  {total_signal}")
    print(f"  Longitud: {len(total_signal)}")
    
    # Ejemplo 3: Codificar y combinar en un paso
    print("\nEjemplo 3: Función combinada")
    print("-" * 50)
    individual, total = encoder.encode_and_combine(messages, codes)
    print(f"Señales individuales: {individual.shape}")
    print(f"Señal total: {total.shape}")
    
    # Ejemplo 4: Agregar ruido
    print("\nEjemplo 4: Señal con ruido")
    print("-" * 50)
    snr_db = 10
    noisy_signal = encoder.add_noise(total_signal, snr_db)
    print(f"Señal limpia: {total_signal}")
    print(f"Señal con ruido (SNR={snr_db}dB): {noisy_signal}")
    print(f"Diferencia (ruido): {noisy_signal - total_signal}")