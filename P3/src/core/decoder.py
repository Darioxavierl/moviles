...
class Decoder:
    ...
    def decode_single_user(self, 
        ...
        #despread_signal = self._despread_spectrum_corre(total_signal, code, n_bits)
    ...
    def _despread_spectrum(self, 
                      ...
                      n_bits: int) -> np.ndarray:
        """
        Desesparcir la señal mediante multiplicación directa con el código.
        
        Para cada bit:
        1. Extraer segmento de la señal (longitud = code_length)
        2. Multiplicar elemento por elemento con el código
        3. Sumar todos los productos (integración)
        ...
        Returns:
            np.ndarray: Valores de integración (uno por bit)
        """
        code_length = len(code)
        integrated_values = np.zeros(n_bits, dtype=np.float64)
        ...
        for i in range(n_bits):
            ...
            # Multiplicación elemento por elemento y suma (integración)
            integrated_values[i] = np.sum(signal_segment * code)
        
        return integrated_values
    
    def _despread_spectrum_corre(self, 
                          total_signal: np.ndarray,
                          code: np.ndarray,
                          n_bits: int) -> np.ndarray:
        """
