...
class CodeGenerator:
    ...
    def _generate_m_sequence(n: int, taps: List[int]) -> np.ndarray:
        ...
        return sequence
    @staticmethod
    def generate_pn_codes(n_users: int, length: Optional[int] = None) -> np.ndarray:
        """
        Genera códigos usando secuencias PN (Pseudorandom Noise / m-sequences).
        
        Las secuencias PN se generan usando LFSR (Linear Feedback Shift Register)
        y tienen buenas propiedades de autocorrelación.
        
        Args:
            n_users: Número de usuarios
            length: Longitud de las secuencias (si None, se calcula automáticamente)
                   Debe ser 2^n - 1 para secuencias maximales
        
        Returns:
            np.ndarray: Matriz de códigos PN de forma (n_users, length)
        
        Example:
            >>> codes = CodeGenerator.generate_pn_codes(4, length=15)
            >>> codes.shape
            (4, 15)
        """
        if n_users < 1:
        ...
        # Determinar longitud si no se especifica
        if length is None:
            # Usar longitud mínima: 2^n - 1
            n = CodeGenerator._calculate_register_length(n_users)
            length = (2 ** n) - 1
        
        # Validar que length sea de la forma 2^n - 1
        valid_lengths = [2**i - 1 for i in range(2, 11)]  # De 3 a 1023
        if length not in valid_lengths:
            nearest = min(valid_lengths, key=lambda x: abs(x - length))
            warnings.warn(
                f"Longitud {length} no es óptima para PN sequences. "
                f"Usando longitud {nearest} en su lugar."
            )
            length = nearest
        ...
        # Polinomios primitivos para diferentes longitudes
        ...
        polynomials = primitive_polynomials.get(n, [[n, n-1]])
        
        pn_codes = []
        
        # Generar códigos con diferentes polinomios y estados iniciales
        for i in range(n_users):
            # Seleccionar polinomio
            poly_idx = i % len(polynomials)
            polynomial = polynomials[poly_idx]
            
            # Generar secuencia con estado inicial diferente
            initial_state = np.zeros(n, dtype=np.int8)
            initial_state[i % n] = 1
            if np.sum(initial_state) == 0:
                initial_state[0] = 1
            
            pn_seq = CodeGenerator._generate_pn_sequence(n, polynomial, initial_state)
            pn_codes.append(pn_seq[:length])
        
        # Convertir a array y normalizar a {-1, +1}
        codes = np.array(pn_codes[:n_users], dtype=np.float64)
        codes = 2 * codes - 1
        
        return codes
    
    @staticmethod
    def _generate_pn_sequence(n: int, taps: List[int], 
                             initial_state: Optional[np.ndarray] = None) -> np.ndarray:
        """
        Genera una secuencia PN usando LFSR con estado inicial configurable.
        
        Args:
            n: Longitud del registro
            taps: Posiciones de los taps para feedback
            initial_state: Estado inicial del registro
        
        Returns:
            np.ndarray: Secuencia PN
        """
        length = (2 ** n) - 1
        sequence = np.zeros(length, dtype=np.int8)
        ...
        if initial_state is None:
            ...
            register = initial_state.copy()
        
        for i in range(length):
            sequence[i] = register[-1]
            
            # Calcular feedback (XOR de los taps)
            feedback = 0
            for tap in taps:
                if tap <= n:
                    feedback ^= register[tap - 1]
    ...
    @staticmethod
    def generate_ovsf_codes(n_users: int, spreading_factor: int) -> np.ndarray:
        """
        Genera códigos OVSF (Orthogonal Variable Spreading Factor).
        
        Los códigos OVSF son una generalización de los códigos Walsh que permiten
        diferentes factores de esparcimiento mientras mantienen ortogonalidad.
        Se usan en sistemas WCDMA (3G).
        
        Args:
            n_users: Número de usuarios
            spreading_factor: Factor de esparcimiento (debe ser potencia de 2)
        
        Returns:
            np.ndarray: Matriz de códigos OVSF (n_users, spreading_factor)
        
        Raises:
            ValueError: Si spreading_factor no es potencia de 2
        
        Example:
            >>> codes = CodeGenerator.generate_ovsf_codes(4, 8)
            >>> codes.shape
            (4, 8)
        """
        if n_users < 1:
        ...
        if not CodeGenerator._is_power_of_2(spreading_factor):
            raise ValueError(
                f"El spreading factor {spreading_factor} debe ser potencia de 2"
            )
        
        # Generar árbol OVSF
        ovsf_tree = CodeGenerator._generate_ovsf_tree(spreading_factor)
        
        # Extraer códigos del nivel deseado
        codes = []
        for i in range(min(n_users, len(ovsf_tree))):
            codes.append(ovsf_tree[i])
        
        # Si necesitamos más códigos, usar códigos de niveles anteriores
        if len(codes) < n_users:
            warnings.warn(
                f"Solo hay {len(ovsf_tree)} códigos OVSF únicos para SF={spreading_factor}. "
                f"Se reutilizarán algunos códigos."
            )
            while len(codes) < n_users:
                codes.append(ovsf_tree[len(codes) % len(ovsf_tree)])
        
        codes_array = np.array(codes[:n_users], dtype=np.float64)
        
        return codes_array
    
    @staticmethod
    def _generate_ovsf_tree(spreading_factor: int) -> List[np.ndarray]:
        """
        Genera el árbol de códigos OVSF hasta el spreading factor especificado.
        
        Construcción recursiva:
        C(2n) = [C(n), C(n)]  y  C'(2n) = [C(n), -C(n)]
        
        Args:
            spreading_factor: Factor de esparcimiento máximo
        
        Returns:
            List[np.ndarray]: Lista de códigos OVSF
        """
        # Código base
        codes = [np.array([1.0], dtype=np.float64)]
        ...
        while current_sf < spreading_factor:
            ...
            for code in codes:
                # Branch 1: [C, C]
                new_code1 = np.concatenate([code, code])
                new_codes.append(new_code1)
                
                # Branch 2: [C, -C]
                new_code2 = np.concatenate([code, -code])
                new_codes.append(new_code2)
